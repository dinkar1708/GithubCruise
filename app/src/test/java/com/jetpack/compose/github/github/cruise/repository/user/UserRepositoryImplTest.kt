...
class UserRepositoryImplTest {
    ...
    fun `test getUserProfile API call rate limit exceeded, api failed`() {
        runTest {
            // Given
            coEvery { mockNetworkDataSource.getUserProfile(userName = "dinkar1708") } throws ApiException(
                ApiErrorResponse(message = "API rate limit exceeded for 134.180.235.148. (But here's the good news: Authenticated requests get a higher rate limit. Check out the documentation for more details.", documentationUrl = "https://docs.github.com/rest/overview/resources-in-the-rest-api#rate-limiting")
            )
            // When
            val resultFlow: Flow<UserProfile> = repository.getUserProfile(userName = "dinkar1708" )
            resultFlow.catch { e ->
                // Then
                TestCase.assertTrue(e is ApiException)
    ...
    fun `test getUserRepositories API call fails`() {
        runTest {
            // Given
            coEvery { mockNetworkDataSource.searchUser(userName = "dinkar1708", page = 1, pageSize = 10) } throws ApiException(
                ApiErrorResponse(message = "API rate limit exceeded for 134.180.235.148. (But here's the good news: Authenticated requests get a higher rate limit. Check out the documentation for more details.", documentationUrl = "https://docs.github.com/rest/overview/resources-in-the-rest-api#rate-limiting")
            )
            // When
