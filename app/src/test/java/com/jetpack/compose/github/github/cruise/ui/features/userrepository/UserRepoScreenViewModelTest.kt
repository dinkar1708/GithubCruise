...
class UserRepoScreenViewModelTest {
    ...
    @Test
    fun `test loadApiData() for uiStateProfile & uiStateRepository state and message on api call failed`() = runTest {
        // Given
        ...
        coEvery { mockUserRepositoryUseCase.filterUserRepositories(false, "dinkar1708", pageNumber, pageSize) } throws ApiError.ApiException(
            ApiErrorResponse(
                message = "API rate limit exceeded for 134.180.235.148. (But here's the good news: Authenticated requests get a higher rate limit. Check out the documentation for more details.",
                documentationUrl = "https://docs.github.com/rest/overview/resources-in-the-rest-api#rate-limiting"
            )
    ...
    @Test
    fun `test loadApiData() for uiStateProfile & uiStateRepository data on api call success`() = runTest {
        // mock data
        ...
        // Given
        coEvery { mockUserRepositoryUseCase.filterUserRepositories(false, login, pageNumber, pageSize) } returns flowOf( userRepoList)
        // Given
    ...
    fun `test filterRepositories() for uiStateRepository data on api call success`() = runTest {
        ...
        coEvery { mockUserRepositoryUseCase.filterUserRepositories(true, login, pageNumber, pageSize) } returns flowOf( userRepoList)
