...
class UserRepoScreenViewModelTest {
    ...
    fun `test loadApiData uiStateProfile - loading cancelled after api call failed`() = runTest {
        ...
        // Given
        coEvery { mockUserRepositoryUseCase.getUserProfile(user.login) } throws ApiException(
    ...
    fun `test loadApiData filterNotForkedUserRepositories - loading cancelled after api call failed`() = runTest {
        ...
        // Given
        coEvery { mockUserRepositoryUseCase.filterNotForkedUserRepositories(user.login, 1, 20) } throws ApiException(
            ApiErrorResponse(message = "API rate limit exceeded for 134.180.235.148. (But here's the good news: Authenticated requests get a higher rate limit. Check out the documentation for more details.", documentationUrl = "https://docs.github.com/rest/overview/resources-in-the-rest-api#rate-limiting")
        )
        // When
        // empty text
